...
import { t as le } from "./TopicQuickPicker-BAKU_tG3.js";
...
function Je({ containerRef: e, player: t, panelId: r, config: i, dataset: a, hiddenSeries: o, xRange: s, logStart: c, loading: l, onViewportStateChange: u }) {
	...
	let C = z(() => {
		...
		if (b.current = {
			x: !1,
			y: !1
		}, C(), e) if (Ae(e), e.setData(e.data, !0), i.xAxisMode === "timestamp" && i.followingViewWidthSec > 0) {
			let t = f.current;
	...
	return B(() => {
		...
		let e = d.current;
		if (!(!e || i.xAxisMode !== "timestamp") && !b.current.x) if (i.followingViewWidthSec > 0) {
			let t = f.current;
			t != null && Number.isFinite(t) && e.setScale("x", {
				min: t - i.followingViewWidthSec,
				max: t
