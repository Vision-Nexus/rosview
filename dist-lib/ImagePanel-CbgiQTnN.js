...
import { t as r } from "./TopicQuickPicker-BlpJfaiH.js";
import { t as i } from "./PanelTopicBar-B9sIi4Ee.js";
import { c as a, d as o, f as s, l as c, m as l, n as u, o as d, r as ee } from "./imageAnnotations-Qh8myJqn.js";
import { o as f, t as te } from "./time-BoEDgjoH.js";
...
function S(e, t = [], n = !1) {
	...
	let i = y(e[r].data), a = i.includes(7) ? [] : w(e, r), o = [...i.includes(7) || a.length > 0 ? a : [...t], ...e.slice(r)], s = r - a.length;
	return s === 0 ? {
		frames: [...e],
		droppedFrames: 0,
		resync: !1
...
function w(e, t) {
	...
	for (let r = t - 1; r >= 0; --r) {
		let t = e[r];
		if (x(t.data) && y(t.data).includes(7)) {
			n = r;
			break;
		}
	}
	return n < 0 ? [] : e.slice(n, t).filter((e) => x(e.data));
}
//#endregion
...
function A(e, t) {
	let n = f(t), r = e.filter((e) => E(e) && f(e.receiveTime) <= n).sort((e, t) => {
	...
	return k(r) < 0 ? [] : j(S(i).frames).map(({ event: e }) => e);
}
function j(e) {
...
async function re(e, t, n, r) {
	...
	for (let i of O) {
		let a = te(r, -i), o = A((await e.getMessagesInTimeRange({
			start: a,
			end: r,
			topics: [n]
...
function ie(e) {
	...
	try {
		...
		return new Worker("data:text/javascript;charset=utf-8," + encodeURIComponent(M), {
			type: "module",
			name: e?.name
		});
	}
...
var P = (a) => {
	let { formatMessage: o } = e(), s = t((e) => e.playerState.activeData?.isPlaying ?? !1), { player: c, panelId: l, setConfig: v, topic: y, annotationTopic: ne, annotationVisible: b, backgroundColor: x, showStatusText: S, fitMode: C, flipHorizontal: w, flipVertical: T, rotation: D, smoothing: O, colorMode: k, colorMap: A, gradient: j, flatColor: M, explicitAlpha: N, minValue: P, maxValue: I } = a, L = m(null), R = m(null), z = m(null), B = m(null), V = m(null), H = m(null), U = m(0), W = m({ phase: "idle" }), G = m(!1), [K, q] = h({ phase: "idle" }), [J, Y] = h(null), X = `${l}:image-main`, Z = `${l}:image-main-h264`, Q = `${l}:image-annotations`, $ = b ? ne.trim() : "";
	p(() => {
		...
		B.current != null && (window.clearTimeout(B.current), B.current = null), z.current && V.current && V.current !== e && (z.current.postMessage({ type: "dispose" }), z.current.terminate(), z.current = null, V.current = null);
		let r = z.current;
		if (!r) {
			r = new ie(), z.current = r;
			let t = e.transferControlToOffscreen();
			V.current = e, r.postMessage({
				type: "init",
				canvas: t
			}, [t]);
		}
		r.onmessage = (e) => {
			...
			let n = t.status;
			se(W.current, n) || (W.current = n, q(n));
		};
		...
		return d.observe(t), window.addEventListener("resize", u), () => {
			c?.(), c = null, window.removeEventListener("resize", u), d.disconnect(), B.current = window.setTimeout(() => {
				let e = z.current;
				e && (e.postMessage({ type: "dispose" }), e.terminate(), z.current = null, V.current = null, W.current = { phase: "idle" }, q({ phase: "idle" }), Y(null), B.current = null);
			}, 0);
		};
	}, [o]), p(() => {
		if (!y) return;
		let e = z.current;
		if (e) return G.current = !1, Y(null), e.postMessage({ type: "reset" }), c.registerHighFrequencyConsumer(X, {
			topic: y,
			lane: "video",
			mode: "latest",
			onLatestMessage: (t) => {
				if (E(t)) {
					if (G.current) return;
					G.current = !0, c.registerHighFrequencyConsumer(Z, {
						topic: y,
						lane: "video",
						mode: "all",
						onMessageBatch: (t) => {
							for (let n of t) E(n) && F(e, n);
						}
					});
				}
				F(e, t);
			...
			c.unregisterHighFrequencyConsumer(X), c.unregisterHighFrequencyConsumer(Z), e.postMessage({ type: "reset" });
		};
	}, [
		c,
		X,
		...
		let e = z.current;
		if (!(!$ || !e)) return c.registerHighFrequencyConsumer(Q, {
			topic: $,
			lane: "video",
			mode: "all",
			onMessageBatch: (t) => {
			...
			c.unregisterHighFrequencyConsumer(Q), e.postMessage({
				type: "overlay",
				overlay: null
			});
		};
		...
		c,
		$
	]), p(() => c.subscribeCurrentTime((e) => {
		z.current?.postMessage({
			type: "playback",
			currentTime: e,
			isPlaying: s
		});
		let t = f(e), n = H.current;
		if (n !== t && (U.current += 1), n != null && t + 5000000n < n) {
			let n = U.current, r = z.current, i = r && y && !G.current ? y : null;
			r && y && G.current ? (r.postMessage({
				type: "reset",
				preserveFrame: !0
			}), re(c, r, y, e)) : r?.postMessage({ type: "reset" });
	...
	return /* @__PURE__ */ _("div", {
		...
		"data-h264-pressure": J?.pressureMode,
		"data-h264-queue-frames": J?.queueFrames,
		"data-h264-dropped-frames": J?.droppedFrames,
		"data-h264-decode-queue": J?.decodeQueueSize,
		"data-h264-media-lag-ms": J?.mediaLagMs,
		"data-h264-resync-count": J?.resyncCount,
		"data-h264-rendered-frames": J?.renderedFrames,
		children: [/* @__PURE__ */ g(i, {
			className: "border-zinc-800 bg-zinc-950",
			children: /* @__PURE__ */ g(r, {
				value: y,
				onChange: (e) => v((t) => ee(e, t)),
				typeIncludes: [...d],
			...
			children: [
				...
				S && K.phase === "ready" && K.width && K.height && /* @__PURE__ */ _("div", {
